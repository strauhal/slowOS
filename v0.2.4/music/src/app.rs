...
impl Library {
    fn config_path() -> PathBuf {
        config_dir("music").join("library.json")
    }
...
impl eframe::App for SlowMusicApp {
    fn update(&mut self, ctx: &Context, _frame: &mut eframe::Frame) {
        ...
        if slowcore::minimize::check_restore_signal("music") {
        ...
        match win_action {
            ...
            WindowAction::Minimize => {
                slowcore::minimize::write_minimized("music", "music");
        ...
        if self.show_about {
            ...
            let resp = egui::Window::new("about music")
                .collapsible(false)
                .resizable(false)
                .default_width(300.0)
                .max_height(max_h)
    ...
    loop {
        match format.next_packet() {
            Ok(packet) => {
                ...
                // with corrupted headers. Isolate the whole decode-and-copy
                // step behind catch_unwind so a panic becomes "skip this
                // packet" rather than a hard process abort.
                //
                // The decode + sample-copy happen in the SAME closure because
                // decoder.decode(&packet) returns a buffer that borrows from
                // `decoder`; letting that reference escape the closure won't
                // compile. So we pull owned Vec<f32> samples out in one step.
                let decode_attempt = std::panic::catch_unwind(
                    std::panic::AssertUnwindSafe(|| -> Option<Vec<f32>> {
                ...
                match decode_attempt {
                    Ok(Some(chunk)) => samples.extend_from_slice(&chunk),
                    Ok(None) => continue,  // normal decode error
                    Err(_)   => continue,  // caught panic
                }
            }

...
impl ProcessManager {
    ...
    pub fn poll(&mut self) -> Vec<String> {
        ...
        for key in &exited {
            ...
            // Extract base binary name (strip "_N" suffix for multi-instance apps)
            let base_binary = if let Some(idx) = key.rfind('_') {
            ...
            // Only mark as not-running if no instances remain
            let still_running = self.children.keys().any(|k| {
                k == base_binary || k.starts_with(&format!("{}_", base_binary))
            });
            if !still_running {
                self.update_running_status(base_binary, false);
            }
        }

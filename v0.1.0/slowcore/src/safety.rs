...
//! non-UTF-8 boundaries, out-of-bounds array access, and unhandled
//! panics in per-frame rendering.
...
/// Bounds-checked slice indexing. Returns None if index is out of bounds.
pub fn safe_get<T>(slice: &[T], index: usize) -> Option<&T> {
    slice.get(index)
}

/// Clamp an index to be within `0..len`, returning None if len is 0.
pub fn clamp_index(len: usize, index: usize) -> Option<usize> {
    if len == 0 {
        None
    } else {
...
mod tests {
    ...
    #[test]
    fn test_clamp_index() {
        assert_eq!(clamp_index(5, 3), Some(3));
        assert_eq!(clamp_index(5, 10), Some(4));
